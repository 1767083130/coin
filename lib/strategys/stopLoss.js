...
let stopLoss = new class{
    constructor(){
        // trade.tradeStatusChangedEvent.register(this,this.onTradeStatusChanged);
        // trade.tradeDelayedEvent.register(this,this.onTradeDelayed);
    }

    * runStopLosses(stepCallBack){
    ...
    * runStopLoss(stopLoss){
        for(let step of stopLoss.steps){
            let accountInfo = yield* account.getSiteAccount(stopLoss.userName,step.site);
            if(!accountInfo){
                continue;
            }
    ...
    * getStopLossLog(step,marketPrice,identifier){
        ...
        */
        ...
        // if(!accountInfo){
        //     accountInfo = yield account.getSiteAccount(stopLossLog.userName,stopLossLog.site);
        //     if(!accountInfo){
        //         return { isSuccess: false, message: "系统错误！"};
        //     }
        // }
    ...
    * runStopLossLog(stopLossLog,marketPrice,identifier){
        ...
        if(operateType == 'trade'){ //需要委托交易
            let account = yield account.getSiteAccount(stopLossLog.userName,stopLossLog.site);
            let symbolAvailable = account.getAvailable(stopLossLog.symbol);
            let symbolTotal =  account.getTotal(stopLossLog.symbol);
            let stepAmount = StopLoss.getStepTradeAmount(symbolTotal,stopLossLog.position); //todo 有点问题,只是考虑了down情况
            ...
            let trade = { 
                ...
                price: consignPrice, 
                tradeType: stopLossStep.tradeType,
                reason: "stoploss", 
                symbol: stopLossLog.symbol
            };

            let res = yield* trade.createTrade(trade);
            if(res.isSuccess){
