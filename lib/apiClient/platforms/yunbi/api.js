'use strict';
const YunbiClient = require('./yunbiClient').YunbiClient;
...
const Site_Name = 'yunbi';

//https://yunbi.com/documents/api/guide
class YunbiApi extends ApiBase {
    constructor(options){
        super(); 
        ...
        this.client = new YunbiClient(newOptions);
    }

    getRealPrice(symbol,callBack){
        //https://yunbi.com//api/v2/depth.json?market=btccny&limit=300
        let market = this._getMarket(symbol);
        ...
        let detail_url = `https://yunbi.com//api/v2/depth.json?market=${market}&limit=300`;
    ...
    buy(options,callBack){
        let apiname = '/api/v2/orders';
        debugger
        var body = {
            market: this._getMarket(options.symbol),
            side: "buy",
    ...
    * fetchRecentOrders(options,stepCallBack){
    ...
    _getCallRes(data){
        ...
        if(data.hasOwnProperty('error')){
            res = { isSuccess: false,body: data, message: data.error.message,code: data.error.code };
        } else {
            res = { isSuccess: true,body: data };
        }
