...
let transfer = new class{
    ...
    * createTransfer(trade,identifier){ //todo
        ...
        if(trade.isTest){
            ...
            //todo 这里要考虑系统的鲁棒性
            let tradeRes;
            let apiOptions = { symbol: newTrade.symbol, amount: newTrade.consignAmount, price: newTrade.price };
            ...
            if(!tradeRes.isSuccess){
                throw new Error(`提交第三方交易平台委托失败。${tradeRes.tradeRes}`);
            }
       ...
       for(let siteOperates of sitesOperates){
           ...
           for(let operate of sitesOperates.operates){
               ...
               for(let i = 0; i < sitesOperates.operates.length; i++){
                   ...
                   if(i != operateIndex){
                       amounts.push(otherOperate.actualAmount); //notice:不能使用tradeAmount有可能是百分比，并且有可能与实际上发生的金额不同
                   } 
               }
